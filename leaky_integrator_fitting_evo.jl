using LinearAlgebra, DifferentialEquations, NPZ, Plots, Evolutionary
...
options = Evolutionary.Options(abstol=0.1, reltol=0.1, iterations=1000, show_trace=true, parallelization=:thread)

# perform optimization
res = Evolutionary.optimize(w->objective_func(w), constr, CMAES(mu=10, lambda=100), options)

# retrieve optimization results
w_winner = Evolutionary.minimizer(res)
